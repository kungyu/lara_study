...
class CompanyController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $company_base = DB::table('company_info')->select('*')->where($where)->first();
        if(is_object($company_base)){
            Session::put('resume_id', $company_base->id);
        }
        return view('company.index',['base' => $company_base,'industry_son'=>$industry_son]);
    }
    ...
    public function company_add(Request $request){
        ...
        $user = $request->user();
        $user_id = $user->id;
        $company_name = trim($request->input('name'));
        $mobile = trim($request->input('mobile'));
        $address = trim($request->input('address'));
        $industry = intval(trim($request->input('industry')));
        ...
        if(preg_match("/^1[34578]{1}\d{9}$/",$mobile) || preg_match("/^(0(10|21|22|23|[1-9][0-9]{2})(-|))?[0-9]{7,8}$/ ", $mobile)){
        ...
        if($result['state'] == 1){
            $where = ['user_id'=>$user_id];
            $region = DB::table('company_info')->select('id')->where($where)->first();
            if(is_object($region) && $region->id){
                $res = DB::table('company_info')
                    ->where($where)
                    ->update(['company_name' => $company_name,  'industry' => $industry, 'status' => $status,
                        'locate' => $locate, 'locate_name' => $locate_name, 'company_desc' => $description,  'mobile' => $mobile,
                        'address' => $address]);
                if($res == 0)
                    $result = array(
                        'state' => 2,
                ...
                $id = DB::table('company_info')->insertGetId(
                    ['company_name' => $company_name, 'user_id' =>$user_id, 'industry' => $industry, 'status' => $status,
                        'locate' => $locate, 'locate_name' => $locate_name, 'company_desc' => $description, 'mobile' => $mobile,
                        'address' => $address, 'random_str' => $random_str, 'created_at' => $created_at]
                );
                if(intval($id) > 0)
    ...
    public function job(Request $request)
    {
        ...
        $jobs = DB::table('company_work')->where($work_where)->get();
        return view('company.job',['jobs' => $jobs]);
    }

    public function job_add(){
        return view('company.job_add');
    }
